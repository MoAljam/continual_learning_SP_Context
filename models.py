...
# NOTE if task specific readout is used, no context is applied to output layer (ro already task specific)
class MLP_SP(nn.Module):
    ...
    def forward(self, x, task_id=0, ro_id=None):
        ...
        for idx, h_fc in enumerate(self.h_fcs):
            ...
            # if using task ro -> skip context on output layer
            if idx == len(self.h_fcs) - 1 and self.use_task_ro:
                continue
            # else apply context for the next layer
            x = x * context
        ...
        elif ro_id == "all:":  # convienience to get out of all heads without repeated calls
            outs = []
            for fc_out in self.fc_outs:
                outs.append(fc_out(x))
            stacked_outs = torch.stack(outs, dim=1)  # shape: (batch_size, n_tasks, output_dim)
            x = stacked_outs

        else:
            x = self.fc_outs[ro_id](x)

        return x

    def binary_context_vec(self, dim, n_tasks, device):
        context = torch.randint(0, 2, (n_tasks, dim), device=device, dtype=torch.float32)
        ...
        if self.use_context:
            for idx, layer_dim in enumerate([self.input_dim] + self.hl_dims):
                if self.context_layers_mask[idx]:
                    context = self.binary_context_vec(layer_dim, self.n_tasks, self.device)
                else:
                    context = torch.ones((self.n_tasks, layer_dim), device=self.device)
